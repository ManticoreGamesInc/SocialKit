...
SOCIAL KIT
	...
	The Social Kit allows you to build a roleplay-driven social space where commerce thrives, creating consumable objects that players can buy, and use as part of the ways to interact with the game.
...
----------------------
	...
	The Social Kit includes one example chair, but this can be used to add interactions to any furniture you put in the game. Furniture automatically puts players in an animation stance when they get close enough, and resets them when they move or jump.
...
Adding Sitting to a Chair
	...
	3. With the ENTIRE TEMPLATE selected, position it so that the cube is situated at the edge of the seat.

Creating a Different Furniture Interaction

	You can make furniture that uses any of the player animation stances by creating new versions of the chair logic.
...
Creating New Consumable Equipment
	...
	There are two different equipment options included with the project, DrinkEquipment and FoodEquipment. You can create your own by duplicating one of these equipments and modifying it.


SHOPS
-----

	Shops are any place where players should be able to buy the consumables. They come in two types: a single item shop which only sells one of the defined consumable items, and category shop which sells all the items in a category. There are three examples in the starter proejct: Food Station, Snow Cone Kiosk, and Coffee Shop.
...
Defining a New Shop
	...
	2. In the folder, there are three example definitions of Consumables: "Food Station", "Snow Cone Kiosk", and "Coffee Shop".
	3. In Project Content, in My Scripts, find "Shop Definition" and drag it into the Consumables folder.
...
Shop Properties
	...
	- itemID: The id of the consumable this shop sells, if it is a single item shop. Leave blank if it is a category shop.
	- category. The category of items this shop sells, if it is a category shop. Leave blank if it is a single item shop.


Creating a New Shop in the World:

The example kiosks in the project are defined by some client-side triggers, and visual geometry and separate collision. You can add your own client geometry and collision into the appropriate areas, or delete both and makes ure that the two client side triggers are correctly positioned for your already existing shop.

	1. In the Interactive Objects folder, open the Shops folder
	2. Duplicate one of the existing shops, "Food Station" for a category shop, or either of the other two for a single item shop.
	3. Open the ClientContext folder to select either the CategoryShop or SingleItemShop scripts, and open the Properties window.
	4. Change the "ShopID" property to match a shop defined in the Definitions folder
	5. Delete or modifty the "Kiosk Geo" group inside the ClientContext folder to change how the shop looks.
	6. Delete or modify the Collider object in the shop's group to change where the collision is. You will need to set its Visibility property to "Force On" in order to see it.
...
1. In the Hierarchy, open the "Interactive Systems" folder and then the "AFKResource" folder inside of it.
2. Select the "AFKResourceServer" script inside, and open the Properties window
3. Change the "ResourceName", "StartValue", "SecondsToWait" and "GainAmount" custom properties to tweak how players get currency in the game.


GRANTING RP
-----------

	RP is a system available to creators in the Perks program to reward players with points that can be redeemed for Core characters, mounts, and emotes. To learn more about RP, see the reference:
...
Adding RP to the Project
	...
	4. In the Consumables folder in the Definitions folder, make sure the consumable item definitions include RP values greater than 0 for the items that should grant RP.
